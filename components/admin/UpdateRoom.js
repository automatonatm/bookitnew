...
import {clearErrors, updateRoom} from "../../redux/actions/adminRoomActions";
import {getRoomDetails, clearErrors as clearRoomError} from "../../redux/actions/roomActions";
...
const NewRoom = () => {
    ...
    const {success, errors, loading} = useSelector(state => state.adminRooms);
    const {loading: loadingRoom, errors: roomErrors, room} = useSelector(state => state.room);
    ...
    useEffect(() => {
        ...
        if (!room || room.id !== id) {
            ...
            setPetAllowed(room.petsAllowed)
        ...
        if (roomErrors) {
            toast.error(roomErrors)
            dispatch(clearRoomError())
        }

    }, [dispatch, success, errors, room, id, roomErrors]);
